...
| **PushOwl** | E-commerce analytics | 8-second queries → ~200 ms, 95th percentile at 30–40 ms, serving 8,500+ tenants |
...
- **Mismatch between needs and tooling** — Our real-world access patterns have been optimised now to be shallow and predictable, so the graph advantage is not actually needed.
- **Operational drag** — We carried a JVM Infrastructure stack, separate pooling, and a second schema lifecycle, which increased on-call burden and slowed iteration
- **Ecosystem alignment** — Postgres is where our embeddings, analytics, and data tooling live, so consolidating reduced glue code and failure modes.
- **Performance clarity** — SQL made query intent, indexing strategy, and optimization decisions easier to reason about and tune over time
- **Long-term sustainability** — Feature delivery and Maintenance risks are lower with a ubiquitous relational stack.
